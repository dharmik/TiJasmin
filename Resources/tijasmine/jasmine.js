var isCommonJS = "undefined" == typeof window && "object" == typeof exports;
...
jasmine.Env.prototype.compareRegExps_ = function(a, b, mismatchKeys, mismatchValues) {
    a.source != b.source && mismatchValues.push("expected pattern /" + b.source + "/ is not equal to the pattern /" + a.source + "/");
    a.ignoreCase != b.ignoreCase && mismatchValues.push("expected modifier i was" + (b.ignoreCase ? " " : " not ") + "set and does not equal the origin modifier");
    a.global != b.global && mismatchValues.push("expected modifier g was" + (b.global ? " " : " not ") + "set and does not equal the origin modifier");
    a.multiline != b.multiline && mismatchValues.push("expected modifier m was" + (b.multiline ? " " : " not ") + "set and does not equal the origin modifier");
    a.sticky != b.sticky && mismatchValues.push("expected modifier y was" + (b.sticky ? " " : " not ") + "set and does not equal the origin modifier");
    return 0 === mismatchValues.length;
};
...
jasmine.Env.prototype.equals_ = function(a, b, mismatchKeys, mismatchValues) {
    ...
    if (a instanceof RegExp && b instanceof RegExp) return this.compareRegExps_(a, b, mismatchKeys, mismatchValues);
...
jasmine.Block.prototype.execute = function(onComplete) {
    if (jasmine.CATCH_EXCEPTIONS) try {
        this.func.apply(this.spec);
    } catch (e) {
        this.spec.fail(e);
    } else this.func.apply(this.spec);
    onComplete();
};
...
jasmine.Matchers.prototype.toHaveBeenCalledWith = function() {
    ...
    this.message = function() {
        ...
        var positiveMessage = "";
        positiveMessage = 0 === this.actual.callCount ? "Expected spy " + this.actual.identity + " to have been called with " + jasmine.pp(expectedArgs) + " but it was never called." : "Expected spy " + this.actual.identity + " to have been called with " + jasmine.pp(expectedArgs) + " but actual calls were " + jasmine.pp(this.actual.argsForCall).replace(/^\[ | \]$/g, "");
...
jasmine.Queue.prototype.addBefore = function(block, ensure) {
    ensure === jasmine.undefined && (ensure = false);
...
jasmine.Spec.prototype.after = function(doAfter) {
    this.queue.isRunning() ? this.queue.add(new jasmine.Block(this.env, doAfter, this), true) : this.afterCallbacks.unshift(doAfter);
};
...
jasmine.Spec.prototype.addBeforesAndAftersToQueue = function() {
    ...
    for (i = 0; this.afterCallbacks.length > i; i++) this.queue.add(new jasmine.Block(this.env, this.afterCallbacks[i], this), true);
    for (suite = this.suite; suite; suite = suite.parentSuite) for (i = 0; suite.after_.length > i; i++) this.queue.add(new jasmine.Block(this.env, suite.after_[i], this), true);
    for (i = 0; runner.after_.length > i; i++) this.queue.add(new jasmine.Block(this.env, runner.after_[i], this), true);
};
